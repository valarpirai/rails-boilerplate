module Concerns::Activities
  ...
  def create_initial_activity
    unless spam?
      if User.current
        ...
        description = 'activities.logged_user_ticket.long'
      end
      create_activity(activity_user || requester, description || 'activities.new_ticket.long', activity_data || {},
        short_descr || 'activities.new_ticket.short')
    end
  end
  ...
  def create_source_activity
    create_activity(User.current, 'activities.source_change.long',
        {'source_name' => source_name}, 'activities.source_change.short')
  end
